...
let time_temp   = Array(100);       // 一時保存用時間格納配列
...
function set_player(x, y, self,st){
    ...
    if(yplay ==1){
        firebase.database().ref(SVR + '/' + st + '/P1').set(prop1);
    }
}
function set_player2(x, y, self, st) {
    ...
    if (yplay == 1) {
        firebase.database().ref(SVR + '/' + st + '/P2').set(prop2);
    }
}
...
function game_over(self, x1, y1, x2, y2){
    ...
    if(yplay == 2){
        if (self.player02.bottom > SCREEN_HEIGHT) {
            AssetManager.get('sound', 'gameover').play();
            self.player02.x = x2;
            self.player02.y = y2;
            console.log('a')
...
function col_y(self){
    ...
    if(self.player02){
    var player2 = self.player02;
    var vy = player2.physical.velocity.y === 0 ? 4 : player2.physical.velocity.y;
    var rect2 = Rect(player2.left, player2.bottom, player2.width, player2.height);
    // ブロックグループをループ
        self.shapeGroup.children.some(function (block) {
            if (Collision.testRectRect(rect2, block)) {
                player2.bottom = block.top;
                player2.physical.velocity.y = -JUMP_POWER;
                AssetManager.get('sound', 'kachi').play();
            }
        });
    }
...
function col_t(self){
    ...
    if(self.player02){
        var player2 = self.player02;
        var rect2 = Rect(player2.left, player2.top, player2.width, player2.height);
        self.shapeGroupT.children.some(function (block) {
            if (Collision.testRectRect(rect2, block)) {
                player2.top = block.bottom;
                player2.physical.velocity.y *= -1;
                AssetManager.get('sound', 'kachi').play();
            }
        });
    }
...
function col_l(self){
    ...
    if(self.player02){
        ...
        self.shapeGroupL.children.some(function (block) {
            if (Collision.testRectRect(rect2, block)) {
                if (player2.physical.velocity.x < 0) {
                    player2.left = block.right;
                    player2.physical.velocity.x *= -1;
                    AssetManager.get('sound', 'kachi').play();
                }
            }
        });
...
function save_states() {
    if(yplay == 1){
        ...
        firebase.database().ref(room + '/' + name + '/' + st_key).once('value', function (data) {
        ...
        let svd_time_value
        firebase.database().ref(SVR + '/' + time_key).once('value', function (data) {
            ...
            firebase.database().ref(SVR + '/' + time_key).set(time_list);
        });
    }
}
...
function p2watch(self, stage){
    const PL2 = self.player02;
    if (yplay != 2) {
        firebase.database().ref(SVR +'/'+ stage + '/P2').on('value', function (data) {
            const v2 = data.val();
            PL2.x = v2.x;
            PL2.y = v2.y;
...
phina.define("Stage01", {
    ...
    init: function () {
        ...
        set_player(50, 450, this, 'stage1');
        set_player2(30, 450, this, 'stage1');
    ...
    update: function (app) {
        ...
        time_temp[1] += app.deltaTime;
        tl.text = 'STAGE ' + 1 + ',  Time : ' + Math.floor(time_temp[1] / 1000) + '.' + ('000' + time_temp[1] % 1000).slice(-3);
        game_over(this, 50, 450, 30, 450);
        p1move(this.player, 'stage1', 1);
        p1move(this.player02, 'stage1', 2);
        this.collisionY();
        this.collisionR();
        this.collisionP();
        this.goalDoor();
...
phina.define("Stage02", {
    ...
    init: function () {
        ...
        set_player(50, 450, this, 'stage2');
        set_player2(30, 450, this, 'stage2');
        this.leftLabel = left_label(40, 600, this);
        this.rightLabel = right_label(600, 600, this);
    ...
    update: function (app) {
        ...
        var tl = this.time_label;
        var key = app.keyboard;
        time_temp[2] += app.deltaTime;
        tl.text = 'STAGE ' + 2 + ',  Time : ' + Math.floor(time_temp[2] / 1000) + '.' + ('000' + time_temp[2] % 1000).slice(-3);
        game_over(this, 50, 450, 30, 450);
        p1move(this.player, 'stage2', 1);
        p1move(this.player02, 'stage2', 2);
        this.collisionY();
        this.collisionR();
        this.collisionP();
        this.goalDoor();
...
phina.define("Stage03", {
    ...
    init: function () {
        ...
        set_goal(550, 496, this);
        set_player(50, 450, this, 'stage3');
        set_player2(30, 450, this, 'stage3');
        this.leftLabel = left_label(40, 600, this);
        this.rightLabel = right_label(600, 600, this);
    ...
    update: function (app) {
        ...
        var tl = this.time_label;
        var key = app.keyboard;
        time_temp[3] += app.deltaTime;
        tl.text = 'STAGE ' + 3 + ',  Time : ' + Math.floor(time_temp[3] / 1000) + '.' + ('000' + time_temp[3] % 1000).slice(-3);
        game_over(this, 50, 450, 30, 450);
        p1move(this.player, 'stage3', 1);
        p1move(this.player02, 'stage3', 2);
        this.collisionY();
        this.collisionR();
        this.collisionL();
        this.collisionT();
...
phina.define("Stage04", {   //stage04
    ...
    init: function () {
        ...
        set_goal(550, 496, this);
        set_player(50, 450, this, 'stage4');
        set_player2(30, 450, this, 'stage4');
        this.leftLabel = left_label(40, 600, this);
        this.rightLabel = right_label(600, 600, this);
    ...
    update: function (app) {
        ...
        var tl = this.time_label;
        var key = app.keyboard;
        time_temp[4] += app.deltaTime;
        tl.text = 'STAGE ' + 4 + ',  Time : ' + Math.floor(time_temp[4] / 1000) + '.' + ('000' + time_temp[4] % 1000).slice(-3);
        game_over(this, 50, 450, 30, 450);
        p1move(this.player, 'stage4', 1);
        p1move(this.player02, 'stage4', 2);
        this.collisionY();
        this.collisionR();
        this.collisionL();
        this.collisionT();
...
phina.define("Stage05", {   //stage05
    ...
    init: function () {
        ...
        set_player(50, 450, this, 'stage5');
        set_player2(30, 450, this, 'stage5');
        this.leftLabel = left_label(40, 600, this);
        this.rightLabel = right_label(600, 600, this);
    ...
    update: function (app) {
        ...
        p1move(this.player02, 'stage5', 2);
        time_temp[5] += app.deltaTime;
        tl.text = 'STAGE ' + 5 + ',  Time : ' + Math.floor(time_temp[5] / 1000) + '.' + ('000' + time_temp[5] % 1000).slice(-3);
        game_over(this, 50, 450, 30, 450);
        this.collisionY();
        this.collisionR();
        this.collisionL();
...
phina.define("Versus", {    //vs stage
    ...
    init: function () {
        ...
        set_player(50, 450, this);  //player01作成
        set_player2(590, 450, this);//player02作成
        this.leftLabel = left_label(40, 600, this);
        this.rightLabel = right_label(600, 600, this);
        ...
        if (yplay != 2) {
            firebase.database().ref(SVR + '/versus/P2').on('value', function (data) {
                const v2 = data.val();
                PL2.x = v2.x;
                PL2.y = v2.y;
                PL2.physical.velocity.x = v2.vx;
        ...
        if (yplay != 1) {
            firebase.database().ref(SVR + '/versus/P1').on('value', function (data) {
                const v1 = data.val();
                PL1.x = v1.x;
                PL1.y = v1.y;
                PL1.physical.velocity.x = v1.vx;
...
phina.define('Player02', {  //playerクラス
    superClass: 'Sprite',
    init: function () {
        this.superInit('ball2', PLAYER_SIZE_X / 2, PLAYER_SIZE_Y / 2);
        this.physical.velocity.x = 0;
        this.physical.gravity.y = GRAVITY;
    },
    update: function (app) {
